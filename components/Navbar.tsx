      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-24">
          ...
          <div className="flex-shrink-0 flex items-center cursor-pointer" onClick={scrollToTop}>
             <h1 className="font-logo text-2xl md:text-3xl tracking-widest text-moto-black">
              ...
              <div className="relative">
                ...
                {isProfileOpen && user && (
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-1 overflow-hidden animate-fade-in-up">
                    ...
                    {isAdmin && (
                       <a href="/admin" onClick={(e) => { e.preventDefault(); alert("En una app real, esto iría al dashboard. Por ahora baja en la página."); document.getElementById('admin-panel-section')?.scrollIntoView({behavior:'smooth'}); setIsProfileOpen(false); }} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-moto-green">
                         Panel Admin
                       </a>
                    )}
                    <button 
          ...
          <div className="md:hidden flex items-center space-x-4">
             ...
             <button
                  onClick={handleUserClick}
                  className={`p-2 rounded-full transition-colors ${user ? 'text-moto-green' : 'text-gray-600'}`}
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                  </svg>
            ...
            {/* Mobile Cart */}
             <button 
                id="mobile-cart-btn"
                onClick={toggleCart}
                className={`relative p-2 text-gray-600 ${isShaking ? 'animate-cart-bounce' : ''}`}
              >
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007zM8.625 10.5a.375.375 0 11-.75 0 .375.375 0 01.75 0zm7.5 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
                </svg>
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-b border-gray-100">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <a href="#" onClick={scrollToTop} className="block px-3 py-2 text-base font-medium text-gray-800 hover:bg-gray-50">INICIO</a>
            <a href="#shop" onClick={handleShopClick} className="block px-3 py-2 text-base font-medium text-gray-800 hover:bg-gray-50">TIENDA</a>
            {user && (
              <button onClick={() => { logout(); setIsOpen(false); }} className="block w-full text-left px-3 py-2 text-base font-medium text-red-600 hover:bg-red-50">
